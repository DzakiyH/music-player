  ...
  methods: {
    ...
    async getSongs() {
      ...
      if (this.songs.length) {
        ...
        const lastDocSnap = await getDoc(doc(songRef, this.songs[this.songs.length - 1].docID))
        ...
        const q = query(collection(db, 'songs'), orderBy('modifiedName'), limit(this.maxPerPage))
        snapshots = await getDocs(q)
      }
